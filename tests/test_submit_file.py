...
class TestValidation:
    ...
    def test_rejects_both_targets(self, tmp_path: Path) -> None:
        svc, _ = _mk_service()
        with pytest.raises(ValueError, match="request_id or ref"):
            svc.submit_file(
                content=_PDF_BYTES,
...
class TestAttachToExisting:
    def test_flips_failed_request_to_ingesting(self, tmp_path: Path) -> None:
        svc, db = _mk_service(crossref=_crossref())
        req = svc.submit({"doi": "10.1/x"})
        db.update(req.id, status=RequestStatus.FAILED, last_error="no OA")

        updated = svc.submit_file(
            content=_PDF_BYTES,
            request_id=req.id,
            filename="paper.pdf",
            inbox=tmp_path,
    ...
    def test_reopens_needs_user(self, tmp_path: Path) -> None:
        svc, db = _mk_service(crossref=_crossref())
        req = svc.submit({"doi": "10.1/x"})
        db.update(req.id, status=RequestStatus.NEEDS_USER)

        updated = svc.submit_file(content=_PDF_BYTES, request_id=req.id, inbox=tmp_path)
        assert updated.status == RequestStatus.INGESTING

    def test_refuses_ingested_request(self, tmp_path: Path) -> None:
        svc, db = _mk_service(crossref=_crossref())
        req = svc.submit({"doi": "10.1/x"})
        db.update(req.id, status=RequestStatus.INGESTED)

        with pytest.raises(ValueError, match="ingested"):
            svc.submit_file(content=_PDF_BYTES, request_id=req.id, inbox=tmp_path)

    def test_refuses_found_in_store(self, tmp_path: Path) -> None:
        svc, db = _mk_service(crossref=_crossref())
        req = svc.submit({"doi": "10.1/x"})
        db.update(req.id, status=RequestStatus.FOUND_IN_STORE)

        with pytest.raises(ValueError, match="found_in_store"):
            svc.submit_file(content=_PDF_BYTES, request_id=req.id, inbox=tmp_path)

    def test_refuses_cancelled(self, tmp_path: Path) -> None:
        svc, db = _mk_service(crossref=_crossref())
        req = svc.submit({"doi": "10.1/x"})
        db.update(req.id, status=RequestStatus.CANCELLED)

        with pytest.raises(ValueError, match="cancelled"):
            svc.submit_file(content=_PDF_BYTES, request_id=req.id, inbox=tmp_path)

    def test_unknown_request_id_errors(self, tmp_path: Path) -> None:
...
class TestCreateFromRef:
    def test_creates_new_request_and_attaches(self, tmp_path: Path) -> None:
    ...
    def test_short_circuits_when_paper_in_store(self, tmp_path: Path) -> None:
...
class TestUrlDownload:
    @respx.mock
    def test_downloads_pdf_from_url(self, tmp_path: Path) -> None:
    ...
    @respx.mock
    def test_404_raises(self, tmp_path: Path) -> None:
    ...
    @respx.mock
    def test_html_error_page_rejected_by_validator(self, tmp_path: Path) -> None:
...
class TestFilenames:
    def test_filename_hint_sanitised(self, tmp_path: Path) -> None:
        svc, _db = _mk_service(crossref=_crossref())
        req = svc.submit({"doi": "10.1/x"})
    ...
    def test_no_filename_falls_back_to_author_year(self, tmp_path: Path) -> None:
        svc, _db = _mk_service(
            crossref=_crossref(authors=["Feng, Z."], year=2024),
        )
        req = svc.submit({"doi": "10.1/abc"})
